...
// Add Label
func createLabel(pod *corev1.Pod, c corev1.PodDNSConfig, labels map[string]string) ([]byte, error) {
	var patch []patchOperation
	patch = append(patch, updateLabel(pod.Labels, labels)...)
	return json.Marshal(patch)
}
...
func (whsvr *WebhookServer) mutate(ar *v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	labels := map[string]string{admissionWebhookAnnotationStatusKey: "injected"}
	patchBytes, err := createPatch(&pod, *whsvr.dnsConfig, annotations, labels)
	if err != nil {
		return &v1beta1.AdmissionResponse{
			Result: &metav1.Status{
				Message: err.Error(),
	...
	// Ajout du label
//	labels := map[string]string{admissionWebhookAnnotationStatusKey: "injected"}
//	patchBytes, err = createLabel(&pod, *whsvr.dnsConfig, labels)
	//if err != nil {
	//	return &v1beta1.AdmissionResponse{
	//		Result: &metav1.Status{
	//			Message: err.Error(),
